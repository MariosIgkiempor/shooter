...
ATLAS_PNG_OUTPUT_PATH :: "data/atlas.png"
...
// The letters every Font bakes. One charset for every typeface: a typeface
// that lacks any of these is a build error, never a silent hole.
LETTERS_IN_FONT :: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890?!&.,_:[]-+/()%"

// The folder within which to look for typefaces (.ttf files)
FONTS_DIR :: "fonts"

// A Typeface is a .ttf in FONTS_DIR, keyed by file stem; each is baked once per
// listed nominal pixel height. Every (typeface, size) pair becomes one entry of
// the generated `Font` enum, named `<Stem>_<size>`. Sizes are a table here, not
// a manifest, so adding one is a one-line code change next to the code that
// bakes it.
Typeface_Spec :: struct {
	stem: string,
	sizes: []int,
}

TYPEFACES :: []Typeface_Spec {
	{"mini_square", {16, 32}},
	{"debug", {18}},
}
...
// One typeface rasterized at one nominal pixel height: a single generated
// `Font` enum entry.
Font_Bake :: struct {
	name: string,
	size: int,
...
main :: proc() {
	...
	Pack_Rect_Item :: struct {
		type: Pack_Rect_Type,
		idx: int,
		font: int, // .Glyph only: index into `fonts`; `idx` is then the rune index
		x: int,
		y: int,
	}
	...
	for spec in TYPEFACES {
		typeface_path := fmt.tprintf("%s/%s.ttf", FONTS_DIR, spec.stem)
		font_data, error := os.read_entire_file_from_path(typeface_path, context.allocator)

		if error != os.ERROR_NONE {
			log.errorf("Typeface %s not found (declared in TYPEFACES)", typeface_path)
			os.exit(1)
		}

		fi: stbtt.fontinfo
		if !stbtt.InitFont(&fi, raw_data(font_data), 0) {
			log.errorf("Typeface %s could not be parsed", typeface_path)
			os.exit(1)
		}

		// every Font shares LETTERS_IN_FONT; a typeface missing any of them
		// would draw `?` at runtime, so refuse to bake it at all
		for r in letters {
			if stbtt.FindGlyphIndex(&fi, r) == 0 {
				log.errorf("Typeface %s has no glyph for %q (every typeface must cover LETTERS_IN_FONT)", typeface_path, r)
				os.exit(1)
			}
		}
		...
		for size in spec.sizes {
			scale_factor := stbtt.ScaleForPixelHeight(&fi, f32(size))
			...
			append(&fonts, Font_Bake {
				name = fmt.tprintf("%s_%v", strings.to_ada_case(spec.stem), size),
				size = size,
				glyphs = glyphs,
			})
		}
	...
	atlas_fonts := make([]Atlas_Font, len(fonts))
	for &af, idx in atlas_fonts {
		af.bake = fonts[idx]
	}
	...
	for rp in pack_rects {
		...
		case .Glyph:
			af := &atlas_fonts[item.font]
			g := af.bake.glyphs[item.idx]
			img := g.image
	...
	fmt.fprintln(f, "// One entry per (Typeface, size) pair baked by atlas_builder's TYPEFACES table.")
	fmt.fprintln(f, "// Text is only ever drawn with a Font; there is no runtime scaling.")
	fmt.fprintln(f, "Font :: enum {")
	for af in atlas_fonts {
		fmt.fprintf(f, "\t%s,\n", af.bake.name)
	}
	fmt.fprintln(f, "}\n")
	...
	fmt.fprintln(f, "Atlas_Font :: struct {")
	fmt.fprintln(f, "\tsize: int, // nominal pixel height the glyphs were rasterized at")
	fmt.fprintln(f, "\tglyphs: []Atlas_Glyph,")
	fmt.fprintln(f, "}")
	fmt.fprintln(f, "")

	fmt.fprintln(f, "atlas_fonts: [Font]Atlas_Font = {")

	for af in atlas_fonts {
		fmt.fprintf(f, "\t.%s = {{\n\t\tsize = %v,\n\t\tglyphs = {{\n", af.bake.name, af.bake.size)
		for ag in af.glyphs {
