...
_Amended._ Both of the enemy's sources named here have moved: `ENEMY_MAX_HEALTH` and the per-Map spawn template are gone, and an Enemy is stamped from its **Enemy Kind**'s row in `enemy_presets` instead ([ADR-0020](0020-enemy-kind-is-the-authored-unit.md)). The paragraph holds unchanged, and now covers one more field: a body copies its Kind's `max_health` as well as its health, precisely so this rule keeps applying — the drawn size derives from that ceiling, and reading it back live would let a retuned Kind shrink a body that is still carrying the health it spawned with. What the body does read back through its `kind` is its colour, which no Tunable can hold the address of anyway.

**Three constants cannot be Tunables and are excluded.** `PLAYER_BAR_MAX_PARTICLES` backs `[N]Resource_Bar_Particle`, `SWORD_ECHO_COUNT` backs `[N]f32`, and `LIGHTNING_BOLT_MAX_SEGMENTS` backs the Lightning Bolt's joint buffer; a fixed-size array bound must stay a compile-time constant. `SWORD_ECHO_COUNT` is only a *capacity* — the per-kind echo count that is actually tunable is `Weapon_Visual.swing_echo_count`, which `draw_game` already clamps against it.
