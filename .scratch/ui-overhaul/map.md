...
- How the finalized component API (once [Component API & styling convention](issues/03-component-api-and-styling.md) resolves) gets validated against the five screens beyond the one prototyped.
- Whether a standalone handoff spec document is needed for the follow-up implementation effort, or whether the map + resolved tickets are handoff enough on their own.
