...
- ~~Reuse `draw_ui_panel` ([hud.odin:23](../../hud.odin)) and `draw_nine_slice` ([renderer.odin:67](../../renderer.odin)) as the vendor-free panel-background primitive rather than reinventing nine-slice rendering.~~ **Superseded** — see [Flat-Rect Menu UI: Transitions & Animation](../menu-ui-polish/map.md)'s [Component API & Reveal/Dismiss state model](../menu-ui-polish/issues/02-component-api-and-state-model.md): nine-slice panel/button art is dropped for flat rectangles (fill + accent border), with every element gaining a Reveal/Dismiss fade. [ADR-0010](../../docs/adr/0010-hand-rolled-menu-ui-no-layout-engine.md) is itself now superseded by [ADR-0014](../../docs/adr/0014-menu-ui-flat-rect-and-reveal-dismiss-animation.md) — its no-generic-layout-engine decision still stands, only the nine-slice-reuse line is overturned.
