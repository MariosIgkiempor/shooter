...
**Amended by [Content expansion](../content-expansion/map.md).** Its [Enemy variety model](../content-expansion/issues/01-enemy-variety-model.md) ticket reopens the **Attack Style expansion** boundary this map ruled out of scope below: Melee/Ranged alone leaves a real roster with only "touches you" and "shoots you", so a difficulty ladder has nowhere to escalate but numbers. New variants are admitted on the bar that each must buy a new *player response* rather than a stat; which ones ship is that map's [Enemy catalog](../content-expansion/issues/04-enemy-catalog.md). The same ticket also revives `Charger` from the archetypes ruled out below (`Turret` turns out to need nothing new — `movement = nil` plus `attack = Ranged` already expresses it), and makes this map's ticket 06 persistence work partly moot: with a Map's composition authoring an `Enemy_Kind` rather than two templates, `Movement_Style_Save`/`Attack_Style_Save` are no longer needed for persistence at all (see [ADR-0020](../../docs/adr/0020-enemy-kind-is-the-authored-unit.md)).
